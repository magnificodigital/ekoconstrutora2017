...
$is = get_posts($args);
foreach ($is as $i) : //Pega os imóveis
	if (isset($empreendimentos) && !empty($empreendimentos)) : //Verifica se os empreendimentos do user não está vazio
		foreach ($empreendimentos as $empreendimento) : //Pega os imóveis que o user está selecionado
			if ($i->ID == $empreendimento->ID) : //Se o imóvel for igual aos do user, então mostra ?>
